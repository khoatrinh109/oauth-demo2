...
const index = ({ application }) => {
    ...
    const handleLogin = async () => {
        const result = await axios.post('/api/oauth2/login', account)
        if (!result.data.success) {
            toast.error(result.data.message)
        } else {
            router.push(`/oauth/confirm?clientId=${application.clientId}`)
        }
    }
...
export const getServerSideProps = async (context) => {
    ...
    const verifyClient = await prisma.application.findFirst({
        where: {
            clientId,
        },
    })
